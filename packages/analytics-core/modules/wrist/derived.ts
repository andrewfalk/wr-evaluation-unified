// src/modules/wrist/utils/calculations.js에서 계산 함수를 이동(로직 무변경). narrative
// 생성(generateNarrative 등)도 계산 결과의 일부라 함께 이관한다(elbow와 동일한 범위).
...
import { BK_TYPE_LABELS, EXPOSURE_TYPE_LABELS, PRESSURE_SOURCE_LABELS, VIBRATION_TOOL_LABELS, WRIST_BRANCH_FIELDS } from './constants';
import {
  createWristTemporalSequence,
  normalizeWristModuleData,
  type WristDiagnosis,
  type WristDiagnosisEntry,
  type WristJobLike,
  type WristModuleShape,
  type WristTemporalSequence,
} from './legacyNormalize';

export type { WristDiagnosis, WristDiagnosisEntry, WristJobEvaluation, WristJobLike, WristModuleShape, WristTemporalSequence } from './legacyNormalize';
export { createWristTemporalSequence, inferWristBkTypeFromDiagnosis, createWristDiagnosisEntry, isWristDiagnosis, normalizeWristModuleData } from './legacyNormalize';
...
export interface WristBurdenThresholds {
  highByCount: number;
  noneMax: number;
  mildAt: number;
  moderateMax: number;
...
const DEFAULT_BURDEN_THRESHOLDS: WristBurdenThresholds = {
  highByCount: 5,
  noneMax: 1,
  mildAt: 2,
  moderateMax: 4,
...
export function getBk2101RepetitionPerHour(entry: WristDiagnosisEntry = {}): number {
...
function getBranchRequiredFields(entry: WristDiagnosisEntry = {}): string[] {
...
function getTemporalRequiredFields(temporalSequence: WristTemporalSequence = {}): string[] {
...
function getConditionalRequiredFields(entry: WristDiagnosisEntry = {}): string[] {
...
function shouldEvaluateExposureDetails(entry: WristDiagnosisEntry = {}): boolean {
  return entry.direct_anatomic_link === 'yes';
}

function getBk2101BranchNarrative(entry: WristDiagnosisEntry = {}): string {
  return [
...
function getBranchNarrative(selectedBkType: string | undefined, entry: WristDiagnosisEntry): string {
...
export function computeTemporalFlags(temporalSequence: WristTemporalSequence = {}): Record<string, boolean> {
...
export function computeDiagnosisFlags(
  ...
  if (entry.direct_anatomic_link === 'no' || (hasValue(entry.direct_anatomic_link) && exposureTypes.length === 0)) {
  ...
  switch (entry.selectedBkType) {
    case 'BK2113': {
      ...
      const strongForce = ['moderate', 'high'].includes(String(entry.force_level));
    ...
    case 'BK2101':
      ...
      if (
        exposureTypes.includes('repetition') &&
...
export function getWristBurdenGrade(
  riskFactorCount = 0,
  flags: Record<string, boolean> = {},
  thresholds: WristBurdenThresholds = DEFAULT_BURDEN_THRESHOLDS,
): string {
...
export function formatCommonExposureTypeText(entry: WristDiagnosisEntry = {}): string {
  return (
...
export function generateNarrative({ entry, temporalSequence: _temporalSequence }: { entry: WristDiagnosisEntry; temporalSequence?: WristTemporalSequence }): string {
  if (!shouldEvaluateExposureDetails(entry)) {
    return [
...
function buildDiagnosisSummary({
  ...
  diagnosis: WristDiagnosis;
  job: WristJobLike;
  entry: WristDiagnosisEntry | undefined;
  temporalSequence: WristTemporalSequence;
}): DiagnosisSummary {
...
export interface WristCalcResult {
  age: number;
  bmi: string | number;
  temporalSequence: WristTemporalSequence;
...
// 전체 계산 — 원본과 동일 로직. `patientData.module`은 원본 syncWristModuleData 대신
// normalizeWristModuleData(legacyNormalize.ts, 이 패키지 전용 독립 구현)로 정규화한다.
export function computeWristCalc(patientData: {
  shared?: Record<string, unknown> & { jobs?: WristJobLike[]; diagnoses?: WristDiagnosis[]; birthDate?: unknown; injuryDate?: unknown; height?: unknown; weight?: unknown };
  module?: WristModuleShape;
  activeModules?: string[];
}): WristCalcResult {
  const shared = patientData.shared || {};
  const jobs = shared.jobs || [];
  const synced = normalizeWristModuleData(patientData.module || {}, jobs, shared.diagnoses || [], patientData.activeModules || []);
...
function isDiagnosisAssessmentComplete(diag: WristDiagnosis | undefined): boolean {
...
export function isWristAssessmentComplete(ctx: CompletionContext): boolean {
  const shared = ctx.shared as { jobs?: WristJobLike[]; diagnoses?: WristDiagnosis[] };
  const jobs = shared.jobs || [];
  const activeModules = ctx.activeModules || [];
  const synced = normalizeWristModuleData(ctx.module as WristModuleShape, jobs, shared.diagnoses || [], activeModules);
...
function scoreEntry(entry: WristDiagnosisEntry | null | undefined): number {
  if (!entry) return 0;
...
export interface BkDiagnosisGroup {
  bkType: string;
  isGrouped: boolean;
  items: Array<{ diagnosis: WristDiagnosis; entry: WristDiagnosisEntry | null }>;
}

export function groupDiagnosesByBkType(diagnoses: WristDiagnosis[], jobEvaluation: { diagnosisEntries?: WristDiagnosisEntry[] }): BkDiagnosisGroup[] {
...
export function pickRepresentativeEntry(items: Array<{ entry: WristDiagnosisEntry | null }>): WristDiagnosisEntry {
