# PR0-B4 필드 매핑표 (Slice -1 산출물, 4차 개정 — 확정)
...
| `knee.diagnosisSide.klGrade` | diagnosis_side | `shared.diagnoses[].klgRight`/`klgLeft` | 완료 |
| `knee.diagnosisSide.confirmedStatus` | diagnosis_side | `shared.diagnoses[].confirmedRight`/`confirmedLeft` | 완료 |
| `knee.diagnosisSide.appliedConfirmedMismatch` | diagnosis_side | `shared.diagnoses[].confirmedCode`/`confirmedName` | 완료 |
| `shoulder.exposure.anyExceeded` | case | `modules.shoulder.jobExtras[].*` 6종 | 완료 |
| `shoulder.diagnosisSide.ellmanClass` | diagnosis_side | `shared.diagnoses[].ellmanRight`/`ellmanLeft` | 완료 |
...
| `spine.vibration.intervalA8Max` | vibration_interval | `vibrationIntervals[].awMax`/`timeValue`/`timeUnit` | 완료 |
| `spine.vibration.intervalExposureHours` | vibration_interval | `vibrationIntervals[].timeValue`/`timeUnit` | 완료 |
...
| `spine.task.weightKg` | task | `modules.spine.tasks[].weight` | 완료 |
| `spine.task.frequencyPerDay` | task | `modules.spine.tasks[].frequency` | 완료 |
...
| `diagnosis.identity.moduleGroup` | diagnosis_side | `shared.diagnoses[].id`/`code`/`name`/`moduleId`(파생 그룹만 반환, 원본 필드 자체는 미노출 — 아래 §1의 별도 행 참고) | 완료 |
...
| `gender` | `patient.identity.gender` | 신규등록 | knee.relatedness.max dependsOn에만 흡수 | case(`patient` 신설) | analyzable | 5 | 예 |
| `height` | `patient.identity.heightCm` | 신규등록 | DEFERRED였음 | case | analyzable | 5 | 예 |
| `weight` | `patient.identity.weightKg` | 신규등록 | DEFERRED였음 | case | analyzable | 5 | 예 |
| `birthDate` | `patient.identity.birthDate` | 신규등록 | 날짜 정책 | case | **filter_only** | 5 | 예 |
...
| `highBloodPressure` | `patient.identity.highBloodPressure` | 신규등록 | DEFERRED였음 | case | analyzable | 5 | 예 |
| `diabetes` | `patient.identity.diabetes` | 신규등록 | DEFERRED였음 | case | analyzable | 5 | 예 |
| `diagnoses`/`jobs`/`videoAnalysis`(컨테이너), `reportOptions`, `videoAnalysis.*` | — | 제외 | 컨테이너/UI전용/영상분석 범위 밖 | — | — | — | 아니오 |
| `diagnoses[].id` | — | **제외**(1차 개정에서 정정) | diagnosis_side grain 엔터티 키 구성요소 — job.id·job_diagnosis의 diagnosisId와 동일 기준으로 통일 | — | — | — | 아니오 |
| `diagnoses[].code` | `diagnosis.identity.code` | 신규등록 | `moduleGroup`은 파생 그룹만 반환, 원본 코드 자체는 미노출 | diagnosis_side | analyzable(high_cardinality) | 6 | 예 |
| `diagnoses[].name` | `diagnosis.identity.name` | 신규등록 | 동일 | diagnosis_side | analyzable(high_cardinality) | 6 | 예 |
| `diagnoses[].moduleId` | — | 보류 | raw moduleId 자체의 노출 가치는 `moduleGroup`(이미 해석된 값) 대비 낮음 — Slice 6 최종 결정 | diagnosis_side | — | 6 | 아니오 |
| `diagnoses[].side` | — | 보류 | 현재 엔터티 키 구성에만 소비 — 독립 노출 여부 Slice 6 재검토 | diagnosis_side | — | 6 | 아니오 |
| `diagnoses[].confirmedCode` | `diagnosis.identity.confirmedCode` | 보류 | `appliedConfirmedMismatch`는 boolean만 반환 — Slice 6 결정 | diagnosis_side | analyzable(high_cardinality) | 6 | 아니오 |
| `diagnoses[].confirmedName` | `diagnosis.identity.confirmedName` | 보류 | 동일 | diagnosis_side | analyzable(high_cardinality) | 6 | 아니오 |
...
| `diagnoses[].assessmentRight`/`assessmentLeft` | `diagnosis.assessment.status` | 신규등록 | DEFERRED였음. 값 도메인: `''`/`'high'`/`'low'`(select, [AssessmentTab.jsx:42](../../../src/core/components/AssessmentTab.jsx#L42)) — `''`는 `not_entered`, 그 외 2값을 categorical/boolean으로 | diagnosis_side | analyzable | 6 | 예 |
| `diagnoses[].reasonRight`/`reasonLeft` | `diagnosis.assessment.lowReason.<옵션>`(옵션 7개, 아래 표) | **신규등록(1차 개정에서 다중선택으로 재분류)** | 체크박스 배열(`AssessmentTab.jsx:53`), 단일값 아님 | diagnosis_side | analyzable(boolean, 옵션별) | 6 | 예 |
...
| `returnConsiderations` | — | 제외 | 자유 서술 | — | — | — | 아니오 |
| `evalMethod` | `spine.case.evalMethod` | 신규등록 | dependsOn에만 흡수 | case | analyzable | 1 | 예 |
| `careerYears`/`careerMonths`/`workDaysPerYear`(spine 자체) | `spine.case.careerYears`/`careerMonths`/`workDaysPerYear` | 신규등록 | 레거시 호환 필드, 존재 자체가 계산 분기에 영향 | case | analyzable | 1 | 예 |
...
| `tasks[].id`/`sharedJobId` | — | 제외 | task grain 엔터티 키 | — | — | — | 아니오 |
| `tasks[].name` | — | 제외 | 자유 서술 | — | — | — | 아니오 |
| `tasks[].posture` | `spine.task.posture` | 신규등록 | 독립 노출 없음(값 도메인 구현 시 원본 옵션 재확인) | task | analyzable(categorical) | 1 | 예 |
| `tasks[].weight` | `spine.task.weightKg` | **기존유지** | §0 참고 | — | — | — | 예 |
| `tasks[].frequency` | `spine.task.frequencyPerDay` | **기존유지** | §0 참고 | — | — | — | 예 |
| `tasks[].timeValue`/`timeUnit`/`correctionFactor` | `spine.task.timeValue`/`timeUnit`/`correctionFactor` | 신규등록 | 독립 노출 없음 | task | analyzable | 1 | 예 |
| `tasks[].force` | — | 제외 | 계산 결과 캐시 | — | — | — | 아니오 |
| `vibrationIntervals[].id`/`sharedJobId` | — | 제외 | vibration_interval grain 엔터티 키 | — | — | — | 아니오 |
| `vibrationIntervals[].name` | — | 제외 | 자유 서술 | — | — | — | 아니오 |
| `vibrationIntervals[].awMin`/`awMax`/`timeValue`/`timeUnit` | `spine.vibration.intervalAwMin`/`intervalAwMax`/`intervalTimeValue`/`intervalTimeUnit` | 신규등록 | `intervalA8Max`/`intervalExposureHours`는 파생 계산 결과, raw 값은 미노출 | vibration_interval | analyzable | 1 | 예 |

## 5. cervical (신설 `cervical_task` grain)
...
## 6. elbow (job_diagnosis grain — Slice 8a/8b/8c)
...
## 7. wrist (elbow와 별개 전수 표 — 필드 자체가 다름)
...
- **기존유지(완료)**: §0의 21개 카탈로그 키 전부 — 원본 raw 필드 기준으로는 klgRight/Left·ellmanRight/Left·confirmedRight/Left·confirmedCode/Name(파생 boolean만)·verticalDistribution·concomitantSpondylosis·jobName(job)·spine.task 2종·spine.vibration 2종(파생)·knee/shoulder/elbow/wrist/cervical/spine 각 대표 파생변수 자체
- **보류(슬라이스 착수 시 최종 결정, 총 10곳)**: diagnosis moduleId/side/confirmedCode/confirmedName 독립노출(6), job workPeriodOverride(2)·spine legacyJobName(1) 노출 가치, elbow/wrist bkSelectionMode(8b) 및 레거시 유출 필드 2종(8c)
- **제외**: LABEL_ONLY_UNUSED 5개(elbow만 존재), 컨테이너/기술ID/자유서술/UI전용/엔터티키 구성요소(diagnosis id 포함, 이번 개정에서 job/job_diagnosis와 기준 통일) 다수
- **신규등록**: 나머지 전부, Slice 1~8c에 배분. 다중선택 필드·게이트 조건·옵션 상수 전부 실제 코드 확인 완료(elbow/wrist/cervical) — 남은 미확인 사항 없음
