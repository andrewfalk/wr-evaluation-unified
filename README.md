...
> **Version:** 5.1.3 | **Status:** 척추 일일선량 공식 정정(MDDM 원형 일치) / 인트라넷 운영 중
...
`calculateDailyDose`가 원형 MDDM 공식 `D_r = √(Σ F²·t / 8h) · 8h`와 단위 불일치(8h 정규화/재곱 누락, 시간을 초로 누적). 동일 입력에 대해 결과 `dailyDoseKNh`가 이전과 달라지나 의도된 정정. 임계치(`dailyDose` 남 2.0 / 여 0.5 kN·h)는 MDDM 원문값이라 유지. 호출자(`computeSpineCalc`)와 표시부(`SpineResultPanel`, `exportHandlers`, core `reportGenerator`·`exportService`)는 반환 키 `dailyDoseKNh`/`hasHighForceTask`만 사용하므로 코드 변경 없음.
