...
function makePool(opts: {
  ...
  const query = vi.fn((sql: string) => {
    ...
    if (sql.includes('SELECT upload_path FROM video_analysis_clips')) {
      return Promise.resolve({ rows: (opts.referencedClips ?? []).map((p) => ({ upload_path: p })) });
    }
...
describe('runVideoClipCleanup', () => {
  ...
  it('미참조 orphan 파일 삭제, 참조 파일은 보존', async () => {
    ...
    const pool = makePool({ referencedClips: [referenced] });
    const res = await runVideoClipCleanup(pool);
