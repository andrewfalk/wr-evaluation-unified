...
- [~] **PR A (6.0-3.5)** 검증 하네스 — annotation 계약(+segments/events) + 오차 계산기 (구현 완료, 리뷰 대기)
  - [x] `shared/contracts/videoAnnotation.ts`(GoldStandardAnnotation+stratification+segments/events) + index 재export
  - [x] `src/core/services/videoValidation.js`(metricKind/numericError/compareFeatureMap/summarizeErrors/binaryMetrics/withinTolerance + EXAMPLE_TOLERANCES)
  - [x] 테스트 26건(계약 12 + 검증 14, mock↔annotation 포함). 전체 593 / lint errors 0(warning 6 기존) / build·tsc OK
  - [x] Codex 반영: gold=0 Infinity 오차율 비마스킹(false positive 방지)·kind/unit 엄격검사·segment endMs≥startMs·annotatedAt datetime·id/ref/annotator min(1)
...
- **검증 리포트 정책(Codex)**: `summarizeErrors`는 `missing_extracted`/`type_mismatch`/`unit_mismatch` status를 집계에서 제외하므로, **리포트 단계에서 이 status들의 count를 별도 집계**해 누락 feature가 조용히 사라지지 않게 한다(통과 판정 시 mismatch 0 확인).
