...
| T43f | **담당의 기반 기본 환자 목록 필터링**: 인트라넷 모드에서 의사/직원은 기본적으로 자신이 담당하는 환자만 목록에 표시하고, 필요 시 조직 전체 환자를 조회·수정할 수 있다. 신규 컬럼 추가 없이 기존 `patient_records.owner_user_id`를 담당의 식별자로 사용하며, 조회 성능용 복합 인덱스만 추가한다. `GET /api/patients`는 `scope=mine|all` 파라미터를 지원하며 기본값은 `mine`이다. `toResponse()`에 `ownerUserId` 필드를 추가하여 프론트가 "내 환자 여부"를 판별할 수 있게 한다. 담당의 변경은 admin 전용 `POST /api/patients/:id/assignment` API로만 허용하고 `patient_assignment_change` audit을 남긴다. 타 담당 환자의 열람·수정은 허용하되, 담당의가 자동 변경되지는 않는다. 프론트는 PatientSidebar에 "내 담당 / 전체" 토글을 추가하고, 전체 조회 시 타 담당 환자에 배지를 표시한다. 새 환자 생성 시 인트라넷 로그인 이름(`session.user.name`)이 담당의(doctorName)로 자동 입력된다. 로컬 모드에서는 scope 필터 미적용. | `server/migrations/0013_patient_owner_index.sql`, `server/src/routes/patients.ts`, `src/core/hooks/usePatientSync.js`, `src/core/hooks/useIntakeWizard.js`, `src/App.jsx`, `src/core/components/PatientSidebar.jsx`, `src/index.css` | (a) 인트라넷 로그인 후 환자 목록 기본이 내 담당 환자만 표시됨 (b) "전체" 전환 시 조직 전체 환자가 나타남 (c) 전체 조회 중 타 담당 환자에 배지 표시됨 (d) 새 환자 생성 시 담당의 필드에 로그인 사용자 이름 자동 입력 (e) 타 담당 환자 수정 후 `owner_user_id`(담당의 FK)가 변경되지 않음을 DB에서 확인 (f) admin만 담당의 변경 가능, 비-admin은 403 (g) `patient_assignment_change` audit 기록 (h) 로컬 모드에서는 scope 토글 미표시, 모든 환자 표시 | T43e | M |
