...
| **일괄입력 양식** | 상시 | 헤더만 있는 빈 양식 `.xlsx` (`exportBatchTemplate`) — 환자 0명에서도 항상 노출 |
| PDF | - | 보고서 미리보기 영역 PDF 변환 |

"일괄입력 양식" 버튼은 상단 툴바와 일괄 Import 모달("빈 양식 다운로드")에 항상 표시되어, 환자가 한 명도 없는 처음 사용자도 작성용 양식을 받을 수 있다.
...
- **등록일 표시(#86)**: `createdAt`(UTC ISO) 문자열 앞 10자를 그대로 잘라 쓰던 사이드바 등록일 표시·정렬/필터·대시보드 최근 활동을, 공용 유틸 `toLocalDateString`으로 통일해 항상 로컬(KST) 캘린더 날짜로 표시하도록 수정.
...
동일 계정 다중 클라이언트 동시 편집 시 "저장 안 됨" 사고에 대응하는, 서버가 실제로 강제하는 환자 단위 편집 락. 기존 revision 기반 optimistic lock은 최종 안전망으로 그대로 유지한다.

- **서버**: `patient_locks` 테이블(migration 0024) — `lease_token_hash`로만 소유권 판정, `client_instance_id` 자기매칭으로 F5 복구 지원, TTL 만료로 스윕 잡 없이 자연 정리. `server/src/db/patientLocks.ts` — acquire/renew/force/peek/release + 쓰기 게이트(`checkLockForWrite`), `patient_records` FOR UPDATE 앵커로 락 관련 트랜잭션 전체를 직렬화. `patients.ts`에 `POST`/`DELETE`/`GET /:id/lock` 신설, patch/delete/assignPatient에 앵커+게이팅 배선, 소프트 삭제 시 락 행 정리. `videoAnalysis.ts` applyJob에도 동일 게이팅. `LOCK_ENFORCEMENT_MODE`(off/observe/enforce) 3단계 롤아웃 플래그 — off는 기존 동작과 완전 동일.
- **프론트**: `usePatientLock`(신규) — 환자를 열면 즉시 acquire 시도, `ttlMs/4` renew 하트비트, force-takeover, 세대번호로 stale 응답 방지, 창 포커스/탭 활성화/30초 주기 안전망 + 지수 백오프(30초~5분)로 'lost'/'held-by-other' 상태 재시도. `usePatientSync`가 락 상태로 활성 환자 push를 게이팅, `flushPatient`는 기존 autosync 큐에 합류. `useSyncStatusSummary`(신규) — 충돌/락상실/저장대기/오프라인 통합 배너용. `App.jsx`에 락↔싱크 브리지, 세션 identity 변화 시 토큰 초기화, held-by-other/lost 배너 + 강제 획득 버튼.
- **외부(코덱스) 리뷰 3라운드**로 배포 전 발견·수정한 결함: TOCTOU 권한 검사 레이스, `clock_timestamp()` 미사용(TTL이 트랜잭션 시작 시각에 고정), observe 모드 감사로그 누락, 만료 경계에서 `peekLock()`의 `null`을 LockRow로 위장하던 버그, `syncPaused` 재진입/local-only 환자 해제 누락, `<StrictMode>` 중복 acquire, 영상 분석 apply 누락 토큰, 프리셋 모달의 락 가드 우회.
- 검증: mock 단위 테스트 90건 + 실제 두 DB 커넥션으로 acquire 경합·FOR UPDATE 직렬화를 검증하는 통합 테스트(`TEST_DATABASE_URL` 있을 때만 실행).

### v6.2.4 (2026-08-01) — 경추/팔꿈치/손목 모듈 저장 무한루프 수정 (#77)

`syncCervicalModuleData`/`syncElbowModuleData`/`syncWristModuleData`가 `JSON.stringify`로 변경 여부를 비교하는데, 정규화 과정에서 객체 키가 고정 순서로 재정렬되면서 원본과 키 순서만 달라도 `changed=true`로 오판했다. PostgreSQL JSONB는 키 삽입 순서를 보존하지 않아 서버 왕복 시마다 이 오판이 반복되어 동일 payload가 계속 재저장되는 무한 루프가 발생했다.
...
- **원인**: `isCervicalAssessmentComplete()`(사이드바 완료 배지 판정)가 화면 표시 로직과 별개로 자체 완료 기준을 갖고 있었는데, 모든 직업의 작업 수가 0건이면 무조건 `false`를 반환하는 조기 반환문이 있었음 — 부담 작업이 실제로 없는 정상 케이스와 "작업 입력을 깜빡한" 케이스를 구분하지 못했음.
...
- **미리보기 탭 명칭·연동 정리**: "EMR 종합소견"/"통합 리포트 초안" 탭을 "종합소견(그룹)"/"종합소견(개별)"로 이름을 바꾸고, 좌측 패턴 그룹/개별 카드 토글을 바꾸면 미리보기 탭도 자동으로 따라가도록 연동(수동으로 다른 탭을 눌러 비교하는 것은 계속 허용). 두 탭 모두 CP949 byte 게이지를 표시(기존에는 그룹 탭에만 있었음). `generateUnifiedEMR(patient, groupOutputOverride)`에 override 파라미터를 추가해 저장된 `reportOptions` 값과 무관하게 그룹/개별 두 형식을 동시에 계산.
- **낮음 사유 그룹 제목 구분**: 상병 상태·업무관련성은 같지만 낮음 사유만 달라 별도 그룹으로 갈린 카드들이 제목이 똑같아 보여 헷갈리던 문제 — 업무관련성이 낮음인 그룹은 제목 뒤에 "낮음 사유 N" 연번을 붙여 구분. 그룹 카드 하단 캐션은 "낮음 사유" → "낮음 사유 상세"로 문구 변경.
- **그룹 카드 구성원 기본 펼침**: 그룹을 펼쳐야 상병 구성원이 보이던 기본값을 반전 — 이제 기본으로 펼쳐져 보이고, "구성원 접기" 버튼으로 감출 수 있음.
- **손목/팔꿈치 요약 가독성**: EMR 종합소견의 손목/팔꿈치 직업별 노출 요약에 직업명이 표시되지 않아 어느 직업 얘기인지 헷갈리던 문제 — 직업명 헤더(`[직업명]`)를 추가하고 직업이 바뀔 때마다 줄바꿈.
...
동일 BK유형으로 묶인 상병 카드가 대표값만 화면에 보여주고 개별 진단의 실제 저장값은 비어있는 채로 남아, 입력 순서(신규 상병 자동 합류 / 개별 카드 첫 BK유형 수동 선택 / 진단코드 변경에 의한 자동추론 변경)에 따라 "다 채웠는데 미완료" 플래그가 뜨던 문제 수정.

- **세 가지 합류 경로 모두 동기화**: 신규 진단이 기존 필드 그룹에 자동 합류, 개별 카드에서 BK유형을 처음 수동 선택, 진단코드 수정으로 자동추론 BK유형이 바뀌는 경우 — 모두 그룹 내 최고점수(가장 많이 채워진) entry를 donor로 찾아 즉시 동기화. 동기화 발생 시 "○○ 상병에 △△ 상병과 동일한 값이 자동 적용되었습니다" 안내 배너 표시(`bkAutoSyncedFrom` 마커, 사용자가 직접 편집하면 자동 해제).
- **대표값 선택 로직 정합화**: 자동 동기화의 donor 선택을 첫 매치(`.find()`)에서 화면 표시(`pickRepresentativeEntry`)와 동일한 최고 점수 기준으로 통일.
- **기존 mismatch 경고 정정**: "통합되었습니다"라는 사실과 다른 문구를 "실제 저장값이 서로 다릅니다"로 정정하고, "표시된 대표값을 전체 상병에 적용" 복구 버튼 추가.
- 검증: `npm run lint` 0 errors, `npm run build:web` 통과, 클라이언트 953 tests pass.

### v6.2.0 (2026-07-30) — 종합평가 패턴 그룹화 + EMR byte 절감 + K-L/Ellman 조건부 표시

상병이 50건을 넘어가면 종합소견 좌측 패널에서 우/좌 개별 클릭이 과도해지고, EMR 종합소견(txtSyth1Cont)이 CP949 3,950byte를 넘어 잘리던 문제를 해결.

- **종합평가 패턴 그룹화**: 상병+방향(평가단위) 단위로 동일한 상태·업무관련성 패턴을 묶어 그룹 단위로 일괄 입력/Undo 할 수 있는 `AssessmentGroupView`/`AssessmentPatternEditor` 신규. 기존 환자는 `shared.reportOptions.groupAssessmentResults` 옵션 미설정 시 출력이 그대로 유지되고, 그룹 뷰 자체는 이 옵션과 무관하게 항상 열려 있어 기존 환자의 미완료 건도 그룹 화면에서 바로 처리 가능.
- **EMR byte 절감**: `emrText.js`로 CP949 byte 계산/절단 유틸 분리 + b6 신체부담 평가 요약본(`buildExposureSummary`) 추가로 b8 축소, 미리보기에 byte 게이지 탭 추가. EMR 전송 전 종합소견이 한도를 초과하면 확인창 표시.
- **K-L/Ellman 조건부 표시**: `supportsKlGrade`/`supportsEllmanClass` — 무릎관절증(M17.0/M17.9)·회전근개(M75.1) 등 조건에 맞는 상병에만 K-L Grade/Ellman Class 입력창 노출(무관한 상병에 불필요한 입력란이 뜨던 문제 해소).
- 검증: 리뷰 지적사항(방향 미선택 상병 그룹 출력 누락, byte 게이지 반올림 오판정, 그룹 전체수정 모달 초기값 미채움 등) 다수 후속 수정 포함. 상세는 커밋 이력(#72) 참조.

### v6.1.6 (2026-07-10) — 환자 목록 의사별 필터 + 비담당 환자 열람성(복사/스크롤) 개선 + EMR 디바이스 rate-limit 수정

- **의사별 필터 드롭다운**: 환자 목록 사이드바의 "내 담당"/"전체" 버튼 토글을 대시보드 통계와 동일한 드롭다운(전체/내 담당/의사별/미배정)으로 교체 — 서버가 이미 지원하던 `GET /api/patients?scope=<의사UUID>`를 프론트에서 노출. admin이 특정 의사·미배정을 선택해도 무조건 `all`로 되돌아가던 기존 버그 수정. 명부 로딩 상태(loading/ready/error)를 도입해 사이드바·대시보드 두 곳의 scope 유효성 가드를 하나로 통합(정책 충돌 제거).
- **비담당 환자 복사·스크롤 허용**: 비담당 환자 조회 시 HTML `inert` 속성으로 전체 콘텐츠를 감싸던 방식이 텍스트 선택(복사)과 스크롤까지 막아 내용이 길면 읽을 수조차 없던 문제 수정 — capture-phase 이벤트 차단(클릭/키보드/입력/붙여넣기/드롭)으로 교체해 편집 상호작용만 막고 복사·스크롤은 허용. 프리셋 적용 경로에 누락돼 있던 담당의 권한 체크도 함께 보강.
- **EMR 디바이스 등록 rate-limit 자기악순환 방지**: `tryRegister()`가 5분 flush 주기 그대로 재등록 확인을 보내 시간당 12회로 서버 한도(5회/시간/사용자)를 항상 초과했고, 429 응답을 하드 실패로 오인해 상태를 error로 강등 — 관리자가 승인해도 클라이언트가 "장치 등록 필요"를 계속 표시하던 문제. 재시도 간격을 20분으로 늘리고 429는 상태를 유지한 채 조용히 재시도하도록 수정.
- **테스트 인프라**: jsdom + `@testing-library/react`/`user-event` 신규 도입(이 저장소 첫 컴포넌트 렌더링 테스트). 관련 로직 단위/렌더 테스트 36건 추가.
- 검증: 클라이언트 872 tests pass, `build:web` 통과. 서버/Electron 무변경 → 인트라넷 설치본 재배포 불필요(서버 이미지만 갱신하면 반영).
...
- **EMR 재해일자 매칭 회귀 수정**: EMR 페이지 VBScript 핸들러 시그니처는 `SSRHPLANLIST_DblClick(iCol, iRow)`=(열, 행)·행 1-based인데 v6.1.3 리팩터에서 `(row, 1)`로 호출해 **항상 1번 행만 로드**되던 문제 → `(0, row)` + 1-based 스캔으로 복구. 사용자가 입력한 재해일자는 보존(미입력 시에만 자동 채움)하여 불일치 폴백이 입력값을 덮어쓰지 않음.
- **일괄입력 빈 양식 다운로드 상시 제공**: 환자가 0명이어도 상단 툴바 "일괄입력 양식" + 일괄 Import 모달 "빈 양식 다운로드"로 헤더만 있는 양식(`exportBatchTemplate`)을 받을 수 있음 — 처음 사용자가 작성용 엑셀을 만들 수 있게.
- 검증: C# 빌드 0 errors / 0 warnings, 클라이언트 812 tests pass. 오프라인 패키지(906MB) 빌드. **Electron/C# 헬퍼 변경 포함 → 인트라넷 설치본 재배포 필요.**

### v6.1.3 (2026-06-30) — EMR 추출 재해일자별 신청건 매칭 + 성별 자동입력 + 직접입력 바이트 한도 완화

- **재해일자별 신청건 매칭**: EMR 추출 시 입력내역(`SSRHPLANLIST`)에 재해일자가 다른 여러 신청건이 있으면 환자 재해일자와 일치하는 건만 로드. 미입력 시 1번 항목 + "재해일자가 다른 신청건이 있습니다" 경고, 일치 건 없으면 1번 항목 + 경고. 배치 모드 stale-grid 방지 witness(`txtSsnNo`) + 변경감지 폴링.
- **성별 자동 추출**: `txtSex`(남/여) 우선, 비어 있으면 주민번호 7번째 숫자(1·3·5·7=남, 2·4·6·8=여)로 파생 → 앱 `male`/`female` 매핑.
- **직접입력 바이트 한도 완화**: 소견서 주입 시 바이트 계산을 UTF-8(한글 3바이트)에서 **CP949 근사(한글 2바이트)·한도 3950**으로 교체 — 한글이 실제 EMR 4000바이트 한도 내에서 더 많이 들어가도록(이전엔 ~2,600바이트에서 일찍 잘림).
- 검증: C# 빌드 0 errors, 클라이언트 812 tests pass.

### v6.1.2 (2026-06-30) — 직업 프리셋 조직 공유

직업 노출 프리셋을 개인(private) 저장에서 **조직 공유(organization)** 로 확장. 저장 모달 공개범위 토글(인트라넷 기본=조직 공유) + 검색/조회 소유자 배지·읽기전용 + 관리자 '프리셋 공유' 탭(양방향 전환) + 마이그레이션 0023(기존 private 백필, 토글 UI와 동일 릴리스로만 배포). **Electron 셸 무변경 → 서버 이미지만 갱신.**

### v6.1.1 (2026-06-28) — 영상분석 처리시간 안정화 + 추론 디바이스(GPU) 토글 (6.0-12)

손목(wholebody) 추론 처리시간 deadline 일관화(클라 폴링이 추론보다 먼저 포기하던 문제 해결) + 조직별 추론 디바이스 토글(auto/cpu/cuda) + 검수 overlay 손 keypoint 가독성 개선. 마이그레이션 0022. **Electron 셸 무변경.** (상세: [docs/VIDEO_ANALYSIS_IMPLEMENTATION_PLAN.md](docs/VIDEO_ANALYSIS_IMPLEMENTATION_PLAN.md))

### v6.1.0 (2026-06-28) — 영상분석 손목(wholebody) + 상지 반복빈도 candidate

근골격계 부담작업 영상분석에 손목/손가락(wholebody 포즈)·상지 반복빈도를 "참고용 candidate"(자동입력 없음)로 추가. 손목은 "손목·손" 프로필 클립만 on-demand wholebody 추론(굴곡=측면·편위=정면 시점 게이트), 어깨/팔꿈치 반복빈도(cycles/min) candidate. 에어갭 모델 manifest role 분리(`pose-body`/`pose-wholebody`). (M4 6.0-10·6.0-11 — 상세는 [docs/VIDEO_ANALYSIS_IMPLEMENTATION_PLAN.md](docs/VIDEO_ANALYSIS_IMPLEMENTATION_PLAN.md))

### v6.0.0 (2026-06-22) — M4 영상분석 시범 운영(참고용) + 에어갭 패키징

근골격계 부담작업 영상분석(자세 추정)을 "참고용 시범 운영"(미검증 배너 + 수정사유 피드백, 자동 게이팅 비활성)으로 활성화하고 에어갭 오프라인 배포 패키지를 완성.

- **영상분석 요약**: 레시피 버전관리(코드 commit·가중치 sha·키포인트 계약 기록, 미검증은 apply fail-closed) + Python 포즈 추론(onnxruntime-cpu)을 app 이미지에 동봉(`.onnx` SHA256 대조). 상세는 [docs/VIDEO_ANALYSIS_IMPLEMENTATION_PLAN.md](docs/VIDEO_ANALYSIS_IMPLEMENTATION_PLAN.md).
- **오프라인 패키지**: `scripts/export-offline-package.ps1` — 실파일 sha 검증·dirty 가드·`release-manifest.json` 생성. compose에 `video_uploads` 볼륨 + 추론 mem/cpu 제한.
