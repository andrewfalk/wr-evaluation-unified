...
export const JOB_METADATA: AnalyticsVariableMetadata[] = [
  ...
  // 정책(엔진 date 미지원)에 따라 filter_only.
  {
    key: 'job.raw.startDate',
    label: '직업력 시작일(원본)',
    group: '직업력 · 공통',
    moduleId: 'job',
    grain: 'job',
    type: 'date',
    provenance: 'raw',
    dependsOn: ['shared.jobs[].id', 'shared.jobs[].startDate'],
    availableAt: 'assessment',
    shownToAssessor: true,
    allowedAnalysisPurposes: ['association'],
    sensitivity: 'non_sensitive',
    formulaFamily: 'job_raw_date',
    supportedFormulaPolicies: [],
  ...
  },
  {
    ...
    type: 'date',
    provenance: 'raw',
    dependsOn: ['shared.jobs[].id', 'shared.jobs[].endDate'],
    availableAt: 'assessment',
    shownToAssessor: true,
    allowedAnalysisPurposes: ['association'],
    sensitivity: 'non_sensitive',
  ...
  },
  {
    ...
    unit: '일/년',
    provenance: 'raw',
    // spine.mddm/shoulder.exposure/cervical.case의 dependsOn에만 있고 job 자체 변수로는
    // 없던 필드(1차 조사에서 확인된 job의 유일한 진짜 잔여 raw 필드).
    dependsOn: ['shared.jobs[].id', 'shared.jobs[].workDaysPerYear'],
    availableAt: 'assessment',
    shownToAssessor: true,
    allowedAnalysisPurposes: ['association'],
    sensitivity: 'non_sensitive',
    formulaFamily: 'job_raw_work_days',
    supportedFormulaPolicies: [],
  },
];
