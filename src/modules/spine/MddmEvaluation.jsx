...
import { calculateCompressiveForce, resolveMddmStatus } from './utils/calculations';
...
// 진동 패널과 라벨 통일 (미평가 / 노출없음 / 노출있음).
const MDDM_STATUS_OPTIONS = [
  ...
  return (
    <div className="panel">
      ...
      <div className="action-group" style={{ marginBottom: 12 }}>
        <span style={{ alignSelf: 'center', marginRight: 8, opacity: 0.8 }}>요추 압박력 노출 여부:</span>
        {MDDM_STATUS_OPTIONS.map(opt => (
          <button
            key={opt.value}
      ...
      {mddmStatus !== 'present' ? (
        <div className="evaluation-empty-state">
          {mddmStatus === 'none'
            ...
            : '요추 압박력 노출 여부를 선택하세요. "노출있음"을 선택하면 작업을 입력할 수 있습니다.'}
        </div>
      ) : (
        <>
          {jobs.length > 1 && (
