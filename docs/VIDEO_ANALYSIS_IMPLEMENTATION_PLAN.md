...
- [~] **PR M3-7b (6.0-7 정리)** keypoints artifact 영속화(`0018`) + TTL·orphan cleanup + 보존 정책 A — **구현 완료, PR 대기**
...
### PR M3-7b — keypoints artifact 영속화 + cleanup + 보존 A (구현 완료, PR 대기)
...
### 6.0-7 업로드 경로 수동 검증 체크리스트 (7a+7b 머지 후, "전 구간 실동작")

> 자동 테스트는 mock(fs/DB/multer)이라 프록시·디스크·실 영상 컨테이너·워커 추론은 못 잡는다. **6.0-7 = 7a+7b가 합쳐져야** 실 업로드 전 구간이 완성되므로, 7b 머지 직후 1회 통합 스모크. **값 타당성은 6.0-B2 몫**(여기선 동작만). 필요 환경: 실 영상 파일(mp4/mov/webm) + Python venv + 네이티브 인트라넷 서버 + `VIDEO_ANALYSIS_ENABLED=true`·`VIDEO_ANALYSIS_UPLOAD_DIR` 설정.

- **Tier 1 (서버 API 직접)**: 실 영상 → `POST /clips`(purpose=analysis_upload) → `POST /clips/:id/upload`
  - [ ] 디스크 스트리밍(메모리 폭발 없음)·`uploadDir/tmp`→atomic rename, `upload_path`/`original_sha256`/`file_state='present'`/`expires_at` 기록
  - [ ] 매직바이트 sniff: 비영상(확장자만 .mp4) → 400 INVALID_MEDIA_TYPE / 크기 초과 → 413
  - [ ] 워커 추론(venv) → `keypoints_path` artifact 생성(좌표만), `result_features` 저장, status review_pending
  - [ ] **retention A**: privacy_first에서 원본 unlink + `upload_path=NULL`+`file_state='deleted'`, artifact는 보존
  - [ ] `npm run cleanup:video`: TTL 만료 원본·artifact 회수 + orphan(미참조·tmp 1h) 회수, fixture 원본 미삭제
- **Tier 3 (웹 UI 라이브, 네이티브 인트라넷)**: 플래그 on → 영상 분석 스텝
  - [ ] 업로드 **진행률** 표시, 완료 후 sample-detect 버튼 활성(canDetect)
  - [ ] sample-detect → TargetPicker 선택 → 분석 실행(워커 실추론) → review_pending → 제안/적용(서버 apply)
  - [ ] Caddy/Express body limit·타임아웃이 실 영상 크기에서 정상(프록시 상향 필요 여부 확인)
  - [ ] 적용 후 원본 삭제·artifact 보존 확인, 비담당 403/non-synced 차단
