...
export function DiseaseSpecificFields({ diagnosisId, selectedBkType, evaluation, onChange, onToggleMultiValue }) {
  const branchRadioGroupClass = 'elbow-branch-radio-grid';
  const branchRadioLabelClass = 'elbow-branch-radio-label';
  const showBk2105PressureSource = evaluation.direct_pressure_level && evaluation.direct_pressure_level !== 'none';
      ...
      <>
        <div className="form-row">
          ...
          <div className="form-group">
            ...
            />
            <div className="result-section-caption">3600 ÷ 주기(초) 기준으로 자동 계산됩니다.</div>
          </div>
        </div>
        <RadioField
          label="단조로운 반복패턴 여부"
      ...
      <>
        ...
        {showBk2103VibrationDetails && (
          <>
            ...
            />
            <div className="form-row">
              <div className="form-group elbow-short-number-field">
                <label>진동 공구 1일 사용 시간</label>
                <div className="elbow-input-with-unit">
                  <input
                    type="number"
                    min="0"
                    step="0.5"
                  ...
                  />
                  <span className="elbow-input-unit">시간</span>
                </div>
              </div>
            </div>
          </>
        ...
        <div className="form-row">
          <RadioField
            label="공구를 강하게 쥐거나 누르면서 사용하는 작업"
  ...
  if (selectedBkType === 'BK2105') {
    return (
      <>
        ...
        />
        <RadioField
          label="팔꿈치를 대고 버티는 작업"
          name={`bk2105_elbow_leaning_${diagnosisId}`}
          value={evaluation.bk2105_elbow_leaning}
          options={YES_NO_OPTIONS}
          onChange={value => onChange('bk2105_elbow_leaning', value)}
          groupClassName={branchRadioGroupClass}
          labelClassName={branchRadioLabelClass}
        />
        {showBk2105PressureSource && (
          <CheckboxGroup
            label="압박 원인"
            values={evaluation.bk2105_pressure_source}
            options={PRESSURE_SOURCE_OPTIONS}
            onToggle={value => onToggleMultiValue('bk2105_pressure_source', value)}
          />
        )}
      </>
    );
