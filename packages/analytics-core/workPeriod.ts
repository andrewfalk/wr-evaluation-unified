// src/core/utils/workPeriod.js에서 전체 이동(순수 함수만, 로직 무변경).
...
export function calculateWorkPeriod(s?: string, e?: string): number {
  if (!s || !e) return 0;
  return Math.max(0, (new Date(e).getTime() - new Date(s).getTime()) / (1000 * 60 * 60 * 24 * 365.25));
}

export function formatWorkPeriod(s?: string, e?: string): string {
  if (!s || !e) return '-';
  const m = Math.round(calculateWorkPeriod(s, e) * 12);
  return `${Math.floor(m / 12)}년 ${m % 12}개월`;
}

export function parseWorkPeriodOverride(str?: string): number {
  if (!str) return 0;
...
export function getEffectiveWorkPeriod(job: JobLike): number {
...
export function getWorkPeriodYearMonth(job: JobLike): { years: number; months: number } {
...
export function getEffectiveWorkPeriodText(job: JobLike): string {
