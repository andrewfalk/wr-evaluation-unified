...
describe.skipIf(!AVAILABLE)('statsEngine <-> analyze.py 실제 프로세스 (smoke)', () => {
  ...
  it('personCount를 포함한 실제 요청은 생략한 요청보다 histogram bin이 더 적게 나온다(200명×3행 브로드캐스트 재현)', () => {
    ...
    const withoutPersonCount = { protocolVersion: 6, variables: [{ key: 'v1', kind: 'continuous', values }] };
    const withPersonCount = { protocolVersion: 6, variables: [{ key: 'v1', kind: 'continuous', values, personCount: 200 }] };
  ...
  function runCorrelationMatrixPerf(label: string, variables: Array<{ key: string; values: number[] }>) {
    ...
    const request = { protocolVersion: 6, correlationMatrix: { method: 'spearman_correlation', variables } };
