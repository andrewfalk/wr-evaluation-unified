...
$wrapperLines = @(
    ...
    'rem TaskName: ' + $TaskName,
    'rem Logs    : ' + $logPath,
    'cd /d "' + $pkgRootAbs + '"',
    'if not exist "' + $logsDir + '" mkdir "' + $logsDir + '"',
    'echo ============================================================ >> "' + $logPath + '"',
    'echo Run start: %DATE% %TIME% >> "' + $logPath + '"',
    'echo TaskName : ' + $TaskName + ' >> "' + $logPath + '"',
    'echo Command  : ' + $exportArgs + ' >> "' + $logPath + '"',
    'powershell.exe -NoProfile -ExecutionPolicy Bypass -File "' + $exportPs1 + '" ' + $exportArgs + ' >> "' + $logPath + '" 2>&1',
    'set RC=%ERRORLEVEL%',
    'echo Run end  : %DATE% %TIME%  (exit %RC%) >> "' + $logPath + '"',
    'echo. >> "' + $logPath + '"',
    'endlocal & exit /b %RC%'
)
