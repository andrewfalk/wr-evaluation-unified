...
import { Pool } from 'pg';
import { acquireLock, forceLock, renewLock, lockPatientAnchor, peekLock } from '../patientLocks';
...
describe.skipIf(!TEST_DB_URL)('patientLocks — 실제 동시 DB 커넥션 (integration)', () => {
  ...
  it('FOR UPDATE 앵커를 쥔 트랜잭션이 커밋되기 전에는 다른 트랜잭션이 앵커를 잡지 못한다(직렬화)', async () => {
    ...
    try {
      ...
      await clientB.query('ROLLBACK'); // 역시 단언보다 먼저

      // 모든 트랜잭션이 안전하게 닫힌 뒤에만 단언한다 — assert가 여기보다 앞에서 실패하면
      // 트랜잭션이 열린 채로 client.release()가 호출돼 커넥션이 풀에 반납되고, 이후 그
      // 커넥션을 재사용하는 다른 쿼리(cleanup 포함)가 조용히 묻힐 수 있다.
  ...
  it('만료된 락은 다음 acquire가 경합 없이 덮어쓴다', async () => {
    ...
    try {
      ...
      try {
        ...
        resultOk = result.ok;
        await client2.query(result.ok ? 'COMMIT' : 'ROLLBACK'); // 단언보다 먼저
      } finally {
        client2.release();
      }
  ...
  it('TTL 판정은 트랜잭션 시작 시각이 아니라 실제 벽시계 시각 기준이다(clock_timestamp 회귀)', async () => {
    // PostgreSQL now()는 트랜잭션 시작 시각에 고정된다 — BEGIN 후 오래 대기(예: FOR UPDATE
    // 앵커 경합)한 트랜잭션이 now()로 TTL을 판정하면, 실제로는 이미 만료된 락을 살아있다고
    // 오판할 수 있다. 이 테스트는 pg_sleep으로 그 지연을 흉내 내 clock_timestamp()가 실제로
    // 쓰이고 있는지 증명한다 — now()로 되돌리면 이 테스트는 실패해야 한다.
    const patientId = await createTestPatient('Race Patient — clock_timestamp regression');
    try {
      const client1 = await pool.connect();
      let leaseToken = '';
      ...
      try {
        await client1.query('BEGIN');
        await lockPatientAnchor(client1 as unknown as QueryRunner, { patientId, orgId });
        const acquired = await acquireLock(client1 as unknown as QueryRunner, {
          patientId, orgId, userId, holderName: 'Clock Test Holder', clientInstanceId: 'eeeeeeee-eeee-eeee-eeee-eeeeeeeeeeee',
        });
        acquiredOk = acquired.ok;
        if (acquired.ok) leaseToken = acquired.leaseToken;
        // 트랜잭션을 먼저 확실히 끝낸 뒤에 단언한다 — 단언이 여기서 먼저 실행되고 실패하면
        // COMMIT 없이 client.release()가 호출되어 트랜잭션이 열린 채로 커넥션이 풀에
        // 반납되고, 이후 pool.query()가 그 커넥션을 재사용할 때 DELETE 등이 커밋되지 않는
        // 채로 조용히 묻혀버릴 수 있다(이번에 실제로 겪은 fixture 잔존 원인).
        await client1.query('COMMIT');
      } finally {
        client1.release();
      }
      expect(acquiredOk).toBe(true);

      // 실제 만료 임박 상황을 만든다: 지금부터 500ms 후 만료.
      await pool.query(
        `UPDATE patient_locks SET expires_at = clock_timestamp() + interval '500 milliseconds' WHERE patient_id = $1`,
        [patientId]
      );
      ...
      try {
        await client2.query('BEGIN');
        // "FOR UPDATE 앵커 대기가 길었다"를 흉내 — 이 트랜잭션의 now()는 지금(만료 전)에
        // 고정되지만, pg_sleep 이후 실제 벽시계는 위에서 설정한 500ms TTL을 이미 넘긴다.
        await client2.query('SELECT pg_sleep(1)');
        const renewed = await renewLock(client2 as unknown as QueryRunner, { patientId, orgId, leaseToken });
        renewedOk = renewed.ok;
        // 단언보다 먼저 트랜잭션을 확실히 끝낸다(위 client1과 동일한 이유).
        await client2.query('ROLLBACK');
      } finally {
        client2.release();
      }
      // clock_timestamp() 기준이므로 이미 만료된 락은 renew가 실패해야 한다.
      // (now()를 썼다면 트랜잭션 시작 시각 기준 "아직 안 지남"으로 오판해 성공했을 것.)
      expect(renewedOk).toBe(false);
    } finally {
      await pool.query('DELETE FROM patient_records WHERE id = $1', [patientId]);
    }
  });
