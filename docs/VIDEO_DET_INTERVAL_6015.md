...
초기 실측은 대체 클립으로 수행했고, 이후 **진단의 원인이 된 실작업 손목 클립 원본**(재활치료,
OneDrive 보관본)을 확보해 라이브 검증·회귀 수정 재검증까지 완료(§실작업 클립 라이브 검증).

| 클립 | 내용 | fps | 성격 |
|---|---|---|---|
| 중추신경계 발달 재활치료3.mp4 (75.4s·2,263f) | 치료사 1인 + 환자(간헐 탐지) | 29.999 | **실작업 손목 클립**(진단 대상 그 클립) — 2인 근접 실전 케이스 |
| pitching01.mp4 (4s·121f) | 투수 1인, 빠른 팔·다리 스윙 | 29.97 | **통제 시나리오**(1인) + carry 최악 케이스(고속 사지) |
| pitching75.mp4 (76.7s·2,299f) | 위를 19회 연결(합성) | 29.97 | **진단 규모 재현**. 4s마다 위치 점프 = sanity guard 의도 검증 |
...
## 실작업 클립 라이브 검증 — 회귀 발견·수정 (detBbox 기억)

dev-stack 라이브 검증(2026-07-05)에서 실작업 손목 클립이 **TARGET_TRACK_LOST**로 실패 — 초기
구현의 실전 회귀. 하네스 진단으로 기전 확정:

- **기전**: 치료사(주 대상, 제자리) 옆에서 환자가 간헐 탐지(off 실행에서도 74%·15개 트랙으로
  깜빡임). 재동기화 매칭이 트래커의 "역산 박스(타이트 — 테이블에 하반신 가림 시 상체만)"와 det
  박스(전신·여유)를 비교하는데 모양 차로 IoU가 낮아, det 프레임 319에서 greedy가 주 대상을 환자
  트랙(t4)에 배정 — **ID 스왑** 후 t1 사망(presence 15% < 50% → TARGET_TRACK_LOST). 계획서가
  경고한 "2인 근접 ID 스왑" 실패 모드의 실전 발현.
- **수정**: 트랙이 **마지막 det 박스를 별도 기억**(`detBbox`, update에서만 갱신 — carry의
  refresh는 불변) + 매칭 점수 = `max(IoU(역산박스), IoU(마지막 det박스))`. 제자리 인물은 det↔det
  비교(IoU≈0.9)가 살아나 단절·스왑이 사라지고, 이동 인물은 역산 박스가 커버. 매 프레임 det(기본
  off)에서는 detBbox==bbox라 **점수·동작 완전 동일**(회귀 diff 0 재확인).
- **수정 후 재검증(같은 클립, GPU)**: 주 대상 t1 presence **15% → 100%**(0..2262 전 구간),
  target 매핑 18/40 실패 → **40/40 동일**, dominant 동일, 속도 71→42.3s(1.68×) 유지.
  feature: **wristRepetitionRate Δ−0.71**(24.19→23.48)·peak Δ+2.2° — 실작업 클립의 편차는 합성
  클립(−7.3)보다 한 자릿수 작음(B2 참고 신호). 환자 쪽 id 파편(t2~t5, 각 ≤4%)은 off 실행(15개
  트랙)과 동질의 탐지 깜빡임으로 대상 분석에 무영향.
- **잔여 리스크(이론)**: 어떤 인물이 간격 중 이탈하고 같은 자리에 다른 인물이 1초 내 진입하면
  detBbox 기억이 잘못 잇는 창이 있음 — max_age(10샘플)로 상한, 촬영 통제 전제에서 비현실적.
...
- **carry 프레임 열화는 사후 복구 불가**(gate는 다음 프레임 det만 강제) — 본질적 비용. 합성 클립
  강제 det 21회(전부 루프 경계), **실작업 클립 10회·손목 loss 동률(50↔50)·peak Δ+2.2°로 수용 범위**
  (§실작업 클립 라이브 검증). gold 라벨 기준 정확도 재확인은 B2에서.
...
- 반복·peak 계열 candidate 피처의 참값 판정은 이 측정으로 불가 — B2 gold 라벨 검증이 게이트
  (단 실작업 클립 실측 Δ는 반복수 −0.71·peak +2.2°로 작음 — 합성 클립의 −7.3은 과대 추정이었음).
