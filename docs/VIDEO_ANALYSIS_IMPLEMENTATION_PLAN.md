...
- [~] **PR M3-7a (6.0-7 백본)** 보존 config 노브 + 서버 multipart 업로드 — **구현 완료, PR 대기**
...
- [ ] **PR M3-7b (6.0-7 정리)** keypoints artifact 영속화(`0018`) + TTL·orphan cleanup + 보존 정책 A(추론 후 원본 삭제·`file_state='deleted'`, artifact는 clip TTL까지)
...
### PR M3-7a — 보존 config 노브 + 서버 multipart 업로드 (구현 완료, PR 대기)
...
### PR M3-7b — keypoints artifact 영속화 + cleanup + 보존 A
- **`0018`**: `jobs.keypoints_path/keypoints_sha256`. 워커가 성공 시 keypoints를 `uploadDir/artifacts/<jobId>.keypoints.json`로 영속(현재 result_features만 저장 → overlay 입력 보존).
- **cleanup 잡**(`videoClipCleanup.ts`, workspaceRetention 패턴): `expires_at` 경과 원본+artifact unlink + orphan 회수. setInterval + CLI.
- **보존 A**: privacy_first 추론 직후 원본 unlink+`upload_path=null`+`file_state='deleted'`, artifact는 **done이 아니라 clip TTL까지** 보존(done=한 번 소비됨, 다중 feature 적용 보호). review_fidelity는 노브만.
