...
### Phase 25: 척추 일일선량 공식 수정 (v5.1.3)

원형 MDDM 공식과의 단위 불일치 정정. 이전 구현은 `sqrt(Σ F²·t_초) / 1000 / 60`으로 8h 정규화·재곱이 누락되고 시간 단위가 초로 혼입되어 결과 차원이 표기(`kN·h`)와 어긋났음.

- **`calculateDailyDose`** (`src/modules/spine/utils/calculations.js`) 재작성:
  - 원형: `D_r = √(Σ F²·t / 8h) · 8h`
  - 시간 단위를 시간(hour)으로 통일 (`totalTimeHours = (timeSeconds × frequency) / 3600`)
  - `Math.sqrt(sumF2T_hour / 8) * 8` → N·h, 이후 `/1000` → kN·h
  - 반환 키 정리: `sumFSquaredT → sumF2T_hour`, `dailyDoseNs → dailyDoseNh` (잘못된 단위명 제거), `dailyDoseKNh` 유지
- **호환성**: `dailyDoseKNh`·`hasHighForceTask` 키만 외부에서 사용되므로 표시부(`SpineResultPanel`, `exportHandlers`, `core/utils/reportGenerator`, `core/utils/exportService`) 코드 변경 없음
- **주의**: 동일 입력에 대해 새 공식의 `dailyDoseKNh` 값이 이전과 달라짐. 기존 환자 데이터 재계산 시 위험도 판정이 바뀔 수 있으나 의도된 정정. 임계치(`thresholds.dailyDose` 남 2.0 / 여 0.5 kN·h)는 MDDM 원문값이므로 유지
...
### v5.1.3 (2026-05-28) — 척추 일일선량 공식 정정

원형 MDDM 공식 `D_r = √(Σ F²·t / 8h) · 8h`와의 단위 불일치 정정. 이전 구현은 `sqrt(Σ F²·t_초) / 1000 / 60`으로 (1) 8h 분모 정규화 누락, (2) 8h 재곱 누락(대신 `/60` 사용, 단위 의미 불명), (3) 시간 단위가 초(s) 상태로 합산되어 표기(`kN·h`)와 차원 불일치.

**변경:**
- `calculateDailyDose` (`src/modules/spine/utils/calculations.js`) 재작성 — 시간 단위를 시간(hour)로 통일 (`totalTimeHours = timeSeconds × frequency / 3600`), `Math.sqrt(sumF2T_hour / 8) * 8 → N·h`, 이후 `/1000 → kN·h`
- 반환 키 정리: `sumFSquaredT → sumF2T_hour` (단위: N²·h), `dailyDoseNs → dailyDoseNh` (잘못된 단위명 제거), `dailyDoseKNh` 유지
- 외부 참조는 `dailyDoseKNh`·`hasHighForceTask` 키만 사용 (`computeSpineCalc`, `SpineResultPanel`, `exportHandlers`, core `reportGenerator`·`exportService`) → 코드 변경 없음
- 임계치(`thresholds.dailyDose` 남 2.0 / 여 0.5 kN·h, `thresholds.lifetimeDose`)는 MDDM 원문값이라 그대로 유지

**영향:** 동일 입력에 대해 `dailyDoseKNh`·`lifetimeDoseMNh` 값이 이전과 달라짐. 기존 환자 데이터 재계산 시 위험도(`risk.level`) 판정이 바뀔 수 있으나 의도된 정정.
