...
const TASK_CHANGE_OPTIONS = [
  ...
  { value: 'increased_load', label: '부하 증가' },
...
function FlagPill({ flag }) {
  return (
    <span className={`elbow-flag-pill tone-${flag.tone || 'neutral'}`} title={flag.description || ''}>
      <span className="elbow-flag-icon">{flag.icon}</span>
...
function MissingList({ title, items }) {
  ...
  return (
    <div className="result-note">
      <strong>{title}</strong>
      <div className="elbow-note-list">{items.join(', ')}</div>
    </div>
  );
}
...
function TemporalSequenceSection({ temporalSequence, onChange }) {
  return (
    <section className="section pattern-surface form-section">
      <div className="section-header elbow-analysis-header">
        <div className="section-title-row">
          <h2 className="section-title"><span className="section-icon">&#x1F4C5;</span>시간적 선후관계</h2>
          <p className="section-description">작업 변화와 증상 발생 순서를 팔꿈치 모듈 전체에 공통으로 한 번 입력합니다.</p>
        </div>
      </div>
      ...
      <div className="job-card">
        ...
        <div className="form-group">
          <label>휴가/업무중단 시 호전</label>
          <div className="radio-group">
            {REST_OPTIONS.map(option => (
              <label key={option.value} className="radio-label">
                <input
                  type="radio"
                  name="elbow_improves_with_rest"
                  value={option.value}
...
function SummaryCard({ summary }) {
  ...
  return (
    <div className="result-detail-card elbow-summary-card">
      <div className="result-card-title">{diag.code || '-'} {diag.name || '팔꿈치 상병'}</div>
      ...
      <div className="result-metric-list">
        ...
        <div className="result-metric-row"><span>하루 작업 비중</span><strong>{entry.shift_share_percent || '-'}%</strong></div>
      </div>
      ...
      <div className="report-preview ai-result-panel elbow-summary-panel">
        ...
        <div className="preview-section elbow-summary-copy">
          ...
          <div className="elbow-summary-conclusion"><strong>종합평가</strong> {summary.riskFactorSentence}</div>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="panel">
      ...
      <div className="section-header">
        <div className="section-title-row">
          <h2 className="section-title"><span className="section-icon">&#x1F4CA;</span>팔꿈치 신체부담 분석</h2>
          <p className="section-description">직업별 요약과 진단별 주요 신호를 시각적으로 정리합니다.</p>
        </div>
      </div>

      <div className="result-panel elbow-result-panel">
        {calc.missingCommonFields?.length > 0 && (
          <div className="elbow-result-spacing">
        ...
        {(calc.jobSummaries || []).length === 0 && (
          <div className="evaluation-empty-state">직업 또는 팔꿈치 상병이 없어 결과를 표시할 수 없습니다.</div>
        )}
