...
인터넷이 차단된 병원 인트라넷 환경에서 `wr-evaluation-unified v5.0.1`을 설치하기 위한
자급 패키지 명세와 단계별 설치 절차입니다.

> **이 문서를 보고 있다면**: 패키지 압축 파일(`wr-evaluation-unified-5.0.1-intranet.zip`)을
...
9. [백업 설정](#9-백업-설정)
10. [패키지 무결성 검증 상세](#10-패키지-무결성-검증-상세)
...
```
wr-evaluation-unified-5.0.1-intranet/    ← 이 디렉터리가 compose 실행 루트
│
│  ── compose 파일 ──────────────────────────────────────────────────────────
...
Expand-Archive wr-evaluation-unified-5.0.1-intranet.zip -DestinationPath C:\wr\

# 패키지 루트로 이동 (이후 모든 명령은 이 디렉터리에서 실행)
Set-Location C:\wr\wr-evaluation-unified-5.0.1-intranet
```

### 4-2. 무결성 검증

이 단계는 **설치 전에 반드시 수행**합니다. 아래 빠른 검증을 통과해야 다음 단계로 진행할 수 있습니다.
자세한 판정 기준과 `release-manifest.json` 확인 방법은 [10절](#10-패키지-무결성-검증-상세)을 참고하세요.

```powershell
# SHA256SUMS의 각 파일을 실제 해시와 비교
# sha256sum 형식: "<해시(64자)><공백 2개><파일명>" — Substring 고정 위치로 파싱해야
# 파일명에 공백/한글/특수문자가 있어도 안전합니다.
$errors = 0
Get-Content SHA256SUMS -Encoding UTF8 | ForEach-Object {
    if ([string]::IsNullOrWhiteSpace($_)) { return }
    $expectedHash = $_.Substring(0, 64).ToLower()
    $filePath     = $_.Substring(66).Trim() -replace '/', '\'
...
> **주의**: `Get-Content`만 사용해서 `-split '\s+'`로 파싱하면 Electron 인스톨러처럼 **공백/한글이 포함된 파일명**이 잘못 분리되어 검증이 실패합니다.
> 반드시 위 예시처럼 `Substring(0,64)` + `Substring(66)`로 **고정 위치 파싱**하고, `Test-Path` / `Get-FileHash`에 `-LiteralPath`를 사용해야 합니다.
...
scp wr-evaluation-unified-5.0.1-intranet.zip admin@192.168.1.100:/opt/wr/
...
# 압축 해제
cd /opt/wr
unzip wr-evaluation-unified-5.0.1-intranet.zip
...
이 단계는 **설치 전에 반드시 수행**합니다. 자세한 판정 기준과 `release-manifest.json`
확인 방법은 [10절](#10-패키지-무결성-검증-상세)을 참고하세요.
...
1. `직업성 질환 통합 평가 프로그램 Setup 5.0.1.exe` 더블클릭
...
### 7-5. 기존 standalone 데이터 마이그레이션 준비

기존 병원 PC에 `v4.2.1` standalone Electron 앱으로 누적한 환자 데이터와 custom preset이 있다면,
서버 연결과 로그인까지 완료한 뒤 [12절](#12-standalone-데이터-마이그레이션)의 절차로 서버에 이전합니다.

> 마이그레이션은 클라이언트 PC의 로컬 디스크에서 데이터를 읽어 서버로 업로드합니다.
> 따라서 **기존 standalone 데이터를 보유한 PC에서 직접 실행**해야 합니다.
...
이 절은 **검증 기준 상세**입니다. 실제 설치 순서에서는 Windows는 [4-2절](#4-2-무결성-검증),
Linux는 [5-2절](#5-2-무결성-검증)에서 먼저 수행합니다. 즉, 10절이 뒤에 있어도
무결성 검증은 설치 전에 끝나야 합니다.

검증에 실패하면 설치를 계속하지 말고 패키지를 다시 생성하거나 재수령하세요. 손상된 Docker 이미지나
변조된 설치 파일로 서버를 기동하면 복구가 더 어려워집니다.

### 10-1. Windows PowerShell 검증

패키지 루트(`SHA256SUMS`가 있는 디렉터리)에서 실행합니다.

```powershell
# sha256sum 형식: "<해시(64자)><공백 2개><파일명>"
# -split '\s+' 로 자르면 파일명에 공백/한글이 있을 때 잘못 분리되므로
# Substring으로 고정 위치 파싱합니다.
$errors = 0
Get-Content SHA256SUMS -Encoding UTF8 | Where-Object { $_.Trim() -and -not $_.Trim().StartsWith('#') } | ForEach-Object {
    if ($_.Length -lt 66) {
...
if ($errors -eq 0) {
    Write-Host "모든 파일 검증 PASS" -ForegroundColor Green
} else {
    Write-Host "$errors 개 파일 검증 실패 — 패키지를 재수령하세요." -ForegroundColor Red
    exit 1
}
```

> **공백/한글 파일명 처리**: Electron 인스톨러(`직업성 질환 통합 평가 프로그램 Setup X.Y.Z.exe`)처럼
> 파일명에 공백·한글이 들어가면 `-split '\s+'`가 첫 공백에서 잘려 경로가 깨집니다.
> sha256sum의 출력 형식은 **해시 64자 + 공백 2개 + 파일명**으로 고정이므로 위 예시처럼
> `Substring(0,64)` / `Substring(66)`로 파싱하고, `Test-Path` · `Get-FileHash` 모두
> `-LiteralPath`를 사용해야 와일드카드 오인 없이 동작합니다.
> `Get-Content`에 `-Encoding UTF8`도 필수입니다 — 한글 경로를 정확히 읽으려면.
...
모든 파일이 `OK`로 표시되어야 합니다. `FAILED`, `No such file`, `WARNING`이 하나라도 나오면
설치를 중단하고 패키지를 다시 받습니다.

### 10-3. manifest 확인

`SHA256SUMS` 검증은 파일 손상 여부를 확인하고, `release-manifest.json`은 패키지가
기대한 릴리즈인지 확인합니다.

**Windows:**
```powershell
$manifest = Get-Content release-manifest.json | ConvertFrom-Json
$manifest | Select-Object version, gitCommit, gitBranch, buildTime
$manifest.images | Select-Object name, tag, imageId
$manifest.electronInstaller
```
...
- `version`: `5.0.1`
- `images[].tag`: 모두 `5.0.1`
- `electronInstaller.included`: `true`
- `electronInstaller.fileName`: 실제 `electron/` 디렉터리의 설치 파일명과 일치
- `gitCommit`: 릴리즈 기록 또는 전달받은 commit SHA와 일치
- `buildTime`: 패키지 생성 시각과 크게 어긋나지 않음

### 10-4. 실패 시 조치

| 증상 | 의미 | 조치 |
|---|---|---|
| `MISSING` / `No such file` | 패키지 파일 누락 | 압축 해제 위치와 파일 복사 누락 여부 확인, 계속 실패하면 재수령 |
| `MISMATCH` / `FAILED` | 파일 손상 또는 다른 버전 파일 혼입 | 설치 중단, 패키지 재생성 또는 재수령 |
| `release-manifest.json`의 버전 불일치 | 의도한 릴리즈가 아님 | 설치 중단, 올바른 패키지 확인 |
| `electronInstaller.included`가 `false` | 클라이언트 설치 파일 누락 | Electron 설치 파일 포함 패키지로 재생성 |
...
```json
{
  ...
  "images": [
    { "name": "wr-app-server",     "tag": "5.0.1", "imageId": "sha256:..." },
    { "name": "wr-backup-monitor", "tag": "5.0.1", "imageId": "sha256:..." },
    { "name": "wr-backup",         "tag": "5.0.1", "imageId": "sha256:..." }
  ],
  "baseImages": ["postgres:16-alpine", "caddy:2-alpine"],
  "electronInstaller": {
    "included": true,
    "fileName": "직업성 질환 통합 평가 프로그램 Setup 5.0.1.exe"
  },
  "checksum": "SHA256SUMS"
}
```
...
## 12. Standalone 데이터 마이그레이션

기존 `v4.2.1` standalone Electron 앱에 저장된 환자 데이터와 custom preset을
인트라넷 서버로 옮기는 절차입니다.

### 12-1. 실행 조건

- 서버 설치, 관리자 초기화, 의료진 계정 생성이 완료되어 있어야 합니다.
- 클라이언트 PC에 `5.0.1` intranet Electron 앱이 설치되어 있어야 합니다.
- 서버 인증서 신뢰 등록이 완료되어야 합니다.
- 기존 standalone 데이터를 가진 **같은 Windows 사용자 계정**으로 앱을 실행해야 합니다.

마이그레이션이 읽는 주요 로컬 파일:

```text
%APPDATA%\직업성 질환 통합 평가 프로그램\wr-eval-data\patients\
%APPDATA%\직업성 질환 통합 평가 프로그램\wr-eval-data\index.json
%APPDATA%\직업성 질환 통합 평가 프로그램\wr-eval-data\autosave.json
%APPDATA%\직업성 질환 통합 평가 프로그램\wr-eval-data\custom-presets.json
```

### 12-2. 실행 절차

1. 기존 standalone 데이터를 가진 병원 PC에서 `5.0.1` intranet Electron 앱 실행
2. 서버 URL 확인: `https://wr.hospital.local`
3. 마이그레이션 대상 의료진 계정으로 로그인
4. **설정** → **데이터 마이그레이션** 섹션으로 이동
5. **이 PC의 로컬 환자 데이터를 서버로 이전** 클릭
6. native 확인 창에서 계속 진행
7. 완료 화면에서 환자 결과와 프리셋 결과를 모두 확인

마이그레이션은 한 번의 실행으로 다음을 함께 처리합니다:

- 로컬 환자 데이터
- 로컬 custom preset (`custom-presets.json`)

### 12-3. 결과 확인

마이그레이션 결과 모달에서 다음 항목을 확인합니다.

| 항목 | 의미 |
|---|---|
| 환자 — 서버 등록 완료 | 서버에 신규 등록된 환자 수 |
| 환자 — 이미 등록됨 | 기존에 서버에 동기화된 환자 수 |
| 환자 — 실패 | 업로드 실패 환자 수 |
| 프리셋 — 서버 등록 완료 | 서버에 신규 등록된 custom preset 수 |
| 프리셋 — 이미 등록됨 | 같은 직업명/분류/설명으로 이미 존재하여 건너뛴 preset 수 |
| 프리셋 — 실패 | 업로드 실패 preset 수 |

환자 실패는 모달에서 재시도할 수 있습니다. 프리셋 실패는 설정의 프리셋 가져오기/내보내기 또는
마이그레이션 재실행으로 확인합니다.

### 12-4. 담당의 배정 확인

기존 standalone 데이터의 `doctorName`이 서버의 의사 계정 이름과 정확히 일치하지 않으면,
환자가 서버에는 등록되지만 현재 로그인 사용자의 **내 담당** 목록에는 보이지 않을 수 있습니다.

이 경우:

1. 환자 목록에서 **전체** 범위로 전환하여 환자가 보이는지 확인
2. 관리자 콘솔에서 미배정 환자 또는 다른 담당 환자를 올바른 의료진에게 배정
3. 클라이언트 앱에서 다시 동기화하거나 앱을 재시작

`5.0.1` 클라이언트는 서버 전체에 환자가 존재하면 `내 담당` 목록이 비어 있어도 빈 랜딩 화면으로
되돌아가지 않도록 처리되어 있습니다.

### 12-5. 재실행 정책

마이그레이션은 여러 번 실행해도 중복 생성을 최대한 피하도록 설계되어 있습니다.

- 환자: 로컬 ID 기반 `Idempotency-Key`로 중복 POST 방지
- 프리셋: 직업명/분류/설명 identity와 서버 멱등 처리로 중복 방지

그래도 운영 전 검증에서는 같은 PC에서 한 번 실행한 뒤 결과를 확인하고, 문제가 있을 때만 재실행하세요.
...
> **활성화 정책 (시범 운영)**: 이 배포는 영상 분석을 **참고용 시범 기능으로 활성화**합니다
> (`.env.production`에 `VIDEO_ANALYSIS_ENABLED=true`). 정확도 검증(6.0-B2)은 진행 중이며, 그 전까지:
> - 분석값은 **참고용** — UI 상단에 "시범 운영" 배너가 상시 표시되고, confidence 자동-차단 임계값은
>   비활성입니다. 값은 **절대 자동 반영되지 않고** 전문의가 직접 확인·확정합니다.
> - 의사가 적용 시 남긴 **수정 사유 메모**는 provenance(`appliedInputs[].editReason`)에 누적되어
>   6.0-B2 검증·개선의 신호로 쓰입니다.
> - 검증 통과 후 임계값을 배선하면(별도 PR) 시범 배너가 사라지고 적극 자동제안으로 전환됩니다.
>
> 기능을 완전히 끄려면 `VIDEO_ANALYSIS_ENABLED=false`(또는 미설정) — 그러면 전 라우트 404·워커
> 미기동·UI 스텝 숨김으로 **완전히 잠깁니다**.
...
### 14-1a. 손목·손 분석(wholebody) on-demand 모델 (6.0-10)

손목/손가락 분석을 쓰는 공정(분석 프로필 **"손목·손"**)만 wholebody 포즈(`rtmw-dw-l-m`, 133점→저장 59)를
추론에 씁니다. 나머지 공정은 기존 body17 그대로입니다.

- **이미지 +약 114MB**: `pose-wholebody` 가중치가 추가로 baking됩니다(`fetch-pose-weights.ps1`이
  detector·pose-body·pose-wholebody 셋 다 받아 manifest에 sha 확정). detector(yolox_tiny)는 body와 공용.
- **무거운 비용은 손목 클립에만**: hand-wrist 클립은 프레임당 CPU·총 처리시간이 posture-basic 대비 **~7×**,
  피크 RAM **약 2.2×**(추론은 클립당 별도 subprocess라 두 모델 동시 상주 없음). 일반 공정 부하는 변화 없음.
- **on-demand 트리거**: 사용자가 공정 프로필을 "손목·손"으로 골라야만 그 클립에 wholebody가 돕니다(기본은
...
> **`%USERPROFILE%\.wslconfig`가 무슨 뜻?** — `%USERPROFILE%`는 현재 로그인한 윈도우 사용자의 홈
> 폴더(보통 `C:\Users\사용자이름`)입니다. 즉 `C:\Users\사용자이름\.wslconfig`라는 설정 파일을 새로
> 만들라는 뜻입니다. 도커가 내부적으로 쓰는 리눅스 VM(WSL2)이 **부팅할 때 이 파일을 읽어** 메모리
> 상한을 정합니다. `wsl --shutdown`은 그 VM을 완전히 껐다 켜서 **바뀐 설정을 다시 읽게** 하는 적용
> 명령입니다(파일만 만들면 적용 안 됨).

#### 14-3-1. 초보자용 step-by-step (실제 서버에서)

1. **서버 RAM 확인** — `Win + R` → `taskmgr` → **성능** 탭 → **메모리**에서 전체 용량(예: 16.0GB) 확인.
2. **메모장으로 설정 파일 작성** — 메모장(Notepad)을 열고 아래 내용을 붙여넣기(16GB 서버 예시):
   ```ini
   [wsl2]
   memory=12GB
   processors=6
   swap=4GB
   ```
   > RAM이 다르면 `memory`는 "전체 RAM − 4GB 정도"로 조정. 예) 32GB 서버 → `memory=26GB`, `processors=8`.
3. **올바른 위치·이름으로 저장(실수 잦은 부분)** — `Ctrl + S` 후:
   - 파일 이름 칸에 따옴표까지 포함해 `"%USERPROFILE%\.wslconfig"` 입력(따옴표가 있어야 `.txt`가 안 붙음)
   - 파일 형식을 `모든 파일 (*.*)`로 변경 → 저장
   - 확인: PowerShell에서 `Get-Content "$env:USERPROFILE\.wslconfig"` 실행해 내용이 보이면 성공.
4. **적용(껐다 켜기)** — PowerShell에서 `wsl --shutdown` 실행 후, **Docker Desktop을 종료했다가 다시 실행**
   (작업표시줄 고래 아이콘 우클릭 → Quit → 재실행). ⚠️ 실행 중인 모든 컨테이너가 멈추므로 **점검 시간에** 수행.
5. **적용 확인** — `wsl -d docker-desktop free -h` 실행 → total 메모리가 설정값(예: 12GB) 근처면 적용 완료.
...
> **주의 — 14-4는 "설치 절차"가 아니라 "측정·기록" 항목입니다.** 운영 방식은 이미 **컨테이너(현재 패키지
> 그대로)**로 정해져 있어 운영자가 따로 설치·전환할 작업은 없습니다. 리허설 때 아래 숫자 3개를 재서
> 기록만 하면 됩니다. 네이티브 Python 전환은 컨테이너가 부족할 때만 검토하는 예비안입니다.

#### 14-4-1. 초보자용 step-by-step (리허설 중 측정)

1. **자원 모니터 켜기** — PowerShell 한 창에서 계속 띄워 둠:
   ```powershell
   docker stats wr-prod-app-1
   ```
   (이름이 다르면 `docker ps`로 실제 컨테이너 이름 확인 후 교체)
2. **1분 클립 분석 1회 실행** — 실제 화면에서 1분 영상 분석 작업을 돌리며 아래 3개를 기록:
   - **처리시간**: 분석 시작 시각 ~ 상태가 `review_pending`으로 바뀐 시각(job 로그)
