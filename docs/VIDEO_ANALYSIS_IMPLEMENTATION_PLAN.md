...
- [ ] **PR D3 (6.0-6b)** 품질검사 + 시점 융합(§8.6.1) + confidence(§8.8) — 하위 분할(상세계획 확정)
  - [ ] **D3a** confidence 세분화(optional `confidenceBreakdown`, 하위호환) + 품질검사(`infer_clip.py`에서 blur·frame-drop 산출 → keypoints `quality` 메타, frame-drop=실 sampledFps 기준) + 게이팅 경로(feature_calc=측정만 → 클라 `videoConfidenceConfig`(번들, **기본 비활성**)로 `convert`가 `autoSuggestAllowed=false` → UI 버튼 비활성, 실 threshold 6.0-B2) + overall=min(존재 성분)
  - [ ] **D3b** 시점 융합(§8.6.1) — 공정당 다중 시점 클립 그룹핑 + clipFeatures 단계 융합 후 per-day + featureKey별 preferred-viewpoint 테이블(**클라 번들 config**, Python feature_config 아님) + 선택 기준=viewpoint 보정 overall + `INTER_VIEW_CONFLICT` + provenance `analysisJobIds[]` 배열화(융합은 클라 `videoAnalysisRun`)
...
- **confidenceBreakdown 집계 규칙 = method별 분기(Medium #3 반영)**: clip→per-day는 breakdown **그대로 운반**; 공정→직업(`videoAggregate`, method=`weightedSum/weightedAvg/or/pick/max`) 집계는 method별 —
  - **weightedSum/weightedAvg**: 성분별 **시간점유 가중평균**.
  - **pick**: **채택된 contribution의 breakdown** 그대로(categorical/candidate·trunkPostureG 등).
  - **max**: **max 값을 만든 contribution의 breakdown**(없으면 보수적 min).
  - **or**: **true를 만든 contribution 기준**(복수면 보수적 min).
  - 공통: `overall`은 집계 후 재계산(min), warning(LOW_*/INTER_VIEW_CONFLICT)은 **합집합**.
- **`autoSuggestAllowed` 집계 전파 규칙(재리뷰 #2 반영, 사유는 `warnings[]`)**: per-day에서 세팅된 `autoSuggestAllowed=false`가 집계에서 **유실 금지** — method별 (사유 코드는 위 warning 합집합 규칙으로 자동 운반, 별도 reason 필드 없음) —
  - **pick/max/or**: **채택/승리 contribution의 `autoSuggestAllowed`** 따라감(그 contribution의 `warnings`도 함께).
  - **weightedSum/weightedAvg**: 기여 source 중 **하나라도 `false`면 결과 `false`(보수적)**, 사유는 false인 source들 `warnings[]` **합집합**.
  - 집계 후 재계산된 `overall`/breakdown을 threshold와 **재평가**해 false 추가 가능(보수적 우선 — 재평가가 true로 되돌리지 않음). *(threshold 기본 비활성이므로 B2 전엔 전 구간 true — 본 규칙은 활성화 대비 forward-proof.)*
- **provenance 배열화(High #3 반영)**: `ProcessFeaturesSchema`에 **`analysisJobIds: string[]`** 추가, 기존 단일 `jobId`는 **deprecated 유지**. **폴백 정규화(재리뷰 보완)**: `analysisJobIds?.length ? analysisJobIds : (jobId ? [jobId] : [])` — 빈 배열/`undefined` jobId에 `[undefined]` 생성 방지. apply provenance·source job consume·`AppliedInput.analysisJobIds` 채우는 경로를 **배열 기반**으로 전환(D1 `sourceAnalysisJobIds` 검증·consume 트랜잭션 포함). **apply 시 정규화 결과 non-empty 검증**(빈 provenance → 적용 거부). `jobId`는 M3에서 제거.
